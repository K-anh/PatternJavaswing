...
public class GUI_NhaCungCap extends JFrame implements ActionListener{
	...
	private NCC_DAO ncc=new NCC_DAO();
	private List<NhaCungCap> dsncc = new ArrayList<NhaCungCap>();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GUI_NhaCungCap framencc = new GUI_NhaCungCap();
//					framencc.setLocationRelativeTo(null);
...
//					frame.setVisible(true);
	...
	public GUI_NhaCungCap() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(GUI_NhaCungCap.class.getResource("/imgs/qls.png")));
...
//	                	dispose();
		...
		JLabel lblNewLabel = new JLabel("Quản lý NCC");
...
//Body Cần code
		...
		if(idNCC < 10) {
			txtMancc.setText("NCC00" + idNCC);
		}else if (idNCC >= 10 && idNCC < 100) {
			txtMancc.setText("NCC0" + idNCC);
		}else if(idNCC >= 100) {
			txtMancc.setText("NCC" + idNCC);
		}
...
//hiển thị click vào ô hoạt động hoặc ngưng hoạt động
		 buttonGroup = new ButtonGroup();
		 rdbtn_đanghđ = new JRadioButton("Đang hoạt động");
		...
		buttonGroup.add(rdbtn_đanghđ);
		 ...
		 rdbtn_ngưnghđ = new JRadioButton("Ngưng hoạt động");
...
//Sự kiện click chuột thì hiện lên TextField
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				int row = table.getSelectedRow();
				assert row >= 0;
				napNhaCungCapVaoTextfields();
			}
		});
		
//nút thêm và sự kiện thêm
		...
		@Override
			public void actionPerformed(ActionEvent e) {
				...
				if(rdbtn_đanghđ.isSelected()) {
					trangThai = true;
				}else if(rdbtn_ngưnghđ.isSelected()) {
					trangThai = false;
				}
				...
				if(checkEmpty()) {
					...
					if(trangThai == true) {
						...
						trangThaiTrongTable = "Ngung Hoat Dong";
					}
					
					Object[] rowData= {txtMancc.getText(), txttenncc.getText(), txtsdt.getText(), txtdiachi.getText(),trangThaiTrongTable};
...
//nút cập nhật và sự kiện cập nhật
		...
		btnCapNhat.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
		     ...
		     // Cập nhật trạng thái của nhà cung cấp trong cơ sở dữ liệu
		        ...
		        boolean trangThai = rdbtn_đanghđ.isSelected();
...
//combobox của ô tìm kiếm tìm theo mã hay tìm theo tên
...
//ô nhập mã và tên của nhà cung cấp để tìm kiếm
		textField_4 = new JTextField();
		textField_4.setFont(new Font("Tahoma", Font.PLAIN, 18));
		textField_4.setBounds(837, 528, 142, 42);
		contentPane.add(textField_4);
		textField_4.setColumns(10);
//nút search và sự kiện tìm
...
//Hiển thị dữ liệu từ lớp danh sách lên table
		ncc =new NCC_DAO();
		table.setRowHeight(25);
		for(NhaCungCap n:ncc.getAllNCC()) {
			...
			String trangThaiTrongTable = trangThai ? "Dang Hoat Dong" : "Ngung Hoat Dong";
			Object[] rowData= {n.getMaNCC(),n.getTenNCC(),n.getSdt(),n.getDiaChi(),trangThaiTrongTable};
			DataModel.addRow(rowData);
		}
 //nút làm mới và sự kiện   
...
//nền	
	...
	private void napNhaCungCapVaoTextfields() {
		int row = table.getSelectedRow();
		if (row >= 0) {
			...
			String tinhTrang = (String) table.getValueAt(row, 4);
			if ("Dang Hoat Dong".equals(tinhTrang)) {
	            rdbtn_đanghđ.setSelected(true);
	            rdbtn_đanghđ.setSelected(false);
	        } else if ("Ngung Hoat Dong".equals(tinhTrang)) {
	            rdbtn_ngưnghđ.setSelected(false);
	            rdbtn_ngưnghđ.setSelected(true);
	        }
	...
	private void timTheoTen() throws SQLException {
		...
		if (listNCC.size() == 0) {
			...
			for (NhaCungCap ncc : listNCC) {
				boolean trangThai = ncc.isTinhTrang();
				String trangThaiTrongTable = trangThai ? "Dang Hoat Dong" : "Ngung Hoat Dong";
				String[] row = {ncc.getMaNCC(), ncc.getTenNCC(), ncc.getSdt(),ncc.getDiaChi(),trangThaiTrongTable };
...
// Kiểm Tra Xác Thực ràng buộc khi nhập
		...
		private Boolean checkEmpty() {
			...
			if (ma.trim().equals("")) {
				return showErrorTx(txtMancc, "Mã NCC không được để trống");

			} else if (!(ma.length() > 0 && ma.matches("^(NCC)[0-9]{3}$"))) {
				JOptionPane.showMessageDialog(null, "Không đúng định dạng" + "\nVui lòng nhập theo VD: NCC001");
				txtMancc.selectAll();
				txtMancc.requestFocus();
				return false;
			}else if (ncc.isMaNCCTonTai(ma)) {
		        JOptionPane.showMessageDialog(this, "Mã NCC đã tồn tại. Vui lòng chọn mã khác.", "Thông báo lỗi", JOptionPane.ERROR_MESSAGE);
		        txtMancc.selectAll();
		        txtMancc.requestFocus();
		        return false;
		    
			} else if (ten.trim().equals("")) {
				return showErrorTx(txttenncc,"Tên NCC không được để trống");

				
				
			} else if (sdt.trim().equals("")) {
				return showErrorTx(txtsdt, "SĐT không được để trống");
			}else if (ncc.isSDTTonTai(sdt)) {
		...
		public int getIDNCC() {
			...
			if(dsncc.isEmpty()) {
				...
				int i = 1;
				for (NhaCungCap dsNVDaDuocTai : dsncc) {
					i++;
				}
				return i;
			}	
