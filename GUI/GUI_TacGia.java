...
public class GUI_TacGia extends JFrame {

	private static final long serialVersionUID = 1L;
	protected static final Component ActionListener = null;
	private JPanel contentPane;
	private JLabel lbltenTG,lblmaTL;
	private JTextField txtTenTG,txtmaTG;
	...
	private TacGia_DAO TG=new TacGia_DAO();
	private List<TacGia> dstg = new ArrayList<TacGia>();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GUI_TacGia frame = new GUI_TacGia();
...
//	                	dispose();
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(GUI_TacGia.class.getResource("/imgs/qls.png")));
		...
		lblNewLabel.setBounds(440, 0, 387, 68);
...
// 		ấn vào hướng dẫn sẽ hiện ra trang web hướng dẫn sử dụng 
		...
		lbltenTG.setBounds(500, 185, 151, 44);
		lbltenTG.setFont(new Font("Tahoma", Font.PLAIN, 18));
		lbltenTG.setForeground(new Color(234, 198, 150));
        contentPane.add(lbltenTG);
		
		 lblmaTL = new JLabel("Mã tác giả");
		 lblmaTL.setBounds(101, 185, 165, 44);
	...
	txtTenTG.setBounds(609, 186, 239, 42);
	txtTenTG.setFont(new Font("Tahoma", Font.PLAIN, 18));
	contentPane.add(txtTenTG);
	txtTenTG.setColumns(10);
	
	txtmaTG = new JTextField();
	txtmaTG.setBounds(200, 186, 239, 42);
	txtmaTG.setFont(new Font("Tahoma", Font.PLAIN, 18));
	txtmaTG.setColumns(10);
	txtmaTG.setEditable(false);
	idTG = getIDTG();
	
	if(idTG < 10) {
		txtmaTG.setText("TG00" + idTG);
	}else if (idTG >= 10 && idTG < 100) {
		txtmaTG.setText("TG0" + idTG);
	}else if(idTG >= 100) {
		txtmaTG.setText("TG" + idTG);
	}
	...
	btnThem.addActionListener(new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(checkEmpty()) {
			TG.addTG(new TacGia(txtmaTG.getText(),txtTenTG.getText()));
			
			//hàm này cập nhật vào bảng
			DataModel.setRowCount(0);
			for(TacGia n:TG.getAllTG()) {
				
					
				Object[] rowData= {n.getMaTacGia(),n.getTenTacGia()};
	...
	private void napLopHocVaoTextfields() {
		int row = table.getSelectedRow();
		if (row >= 0) {
			txtmaTG.setText((String) table.getValueAt(row, 0));
			txtTenTG.setText((String) table.getValueAt(row, 1));
	...
	private void updateJComboBox() {
		cboModelTK.addElement("Mã Tác Giả");
		cboModelTK.addElement("Tên Tác Giả");
	...
	// Kiểm Tra Xác Thực ràng buộc
		private Boolean checkEmpty() {
			...
			if (maTG.trim().equals("")) {
				return showErrorTx(txtmaTG, "Mã  Tác Giả không được để trống");

			}else if (!(maTG.length() > 0 && maTG.matches("^(TG)[0-9]{3}$"))) {
				JOptionPane.showMessageDialog(null, "Không đúng định dạng" + "\nVui lòng nhập theo VD: TG001");
				txtmaTG.selectAll();
				txtmaTG.requestFocus();
				return false;
			} else if (TG.isMaTGTonTai(maTG)) {
		        JOptionPane.showMessageDialog(this, "Mã TG đã tồn tại. Vui lòng chọn mã khác.", "Thông báo lỗi", JOptionPane.ERROR_MESSAGE);
		        txtmaTG.selectAll();
		        txtmaTG.requestFocus();
		        return false;
			}
			else if (ten.trim().equals("")) {
				return showErrorTx(txtTenTG,"Tên TG không được để trống");
			}
				return true;
  ...
  //Hàm tìm kiếm
	...
	private void timTheoTen() throws SQLException {
		...
		if (dstg.size() == 0) {
			JOptionPane.showMessageDialog(null, "Không tìm thấy tên TG  theo ten! ");
	...
	private void timTheoMa() {
		...
		String matg = textField_4.getText();
		 ...
		 TacGia_DAO daotg=new TacGia_DAO();
		TacGia tg=new TacGia();
		tg=daotg.getTGTheoMa(matg) ;
