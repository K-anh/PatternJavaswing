...
public class GUI_TheLoai extends JFrame {
	...
	private TheLoai_DAO TL=new TheLoai_DAO();
	private List<TheLoai> dstl = new ArrayList<TheLoai>();
	...
	public GUI_TheLoai() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(GUI_TheLoai.class.getResource("/imgs/qls.png")));
...
//	                	dispose();
		...
		lblNewLabel.setBounds(420, 0, 387, 68);
...
// 		ấn vào hướng dẫn sẽ hiện ra trang web hướng dẫn sử dụng 
		...
		lbltenTL.setBounds(448, 181, 151, 44);
        ...
        contentPane.add(lbltenTL);
		 ...
		 lblmaTL.setBounds(63, 181, 165, 44);
	...
	if(idTL < 10) {
		txtmaTL.setText("TL00" + idTL);
	}else if (idTL >= 10 && idTL < 100) {
		txtmaTL.setText("TL0" + idTL);
	}else if(idTL >= 100) {
		txtmaTL.setText("TL" + idTL);
	}
	...
	String[] headers= "Mã thể loại;Tên Thể Loại".split(";");
	...
	btnThem.addActionListener(new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(checkEmpty()) {
			TL.addTL(new TheLoai(txtmaTL.getText(),txtTenTL.getText()));
	...
	private void updateJComboBox() {
		cboModelTK.addElement("Tìm theo mã thể loại");
		cboModelTK.addElement("Tìm theo tên thể loại");
	...
	public int getIDTL() {
		...
		if(dstl.isEmpty()) {
			...
			int i = 1;
			for (TheLoai dsNVDaDuocTai : dstl) {
				i++;
			}
			return i;
		}	
	...
	// Kiểm Tra Xác Thực ràng buộc
			private Boolean checkEmpty() {
				...
				if (maTl.trim().equals("")) {
					return showErrorTx(txtmaTL, "Mã  Thể Loại không được để trống");

				}else if (!(maTl.length() > 0 && maTl.matches("^(TL)[0-9]{3}$"))) {
					JOptionPane.showMessageDialog(null, "Không đúng định dạng" + "\nVui lòng nhập theo VD: TL001");
					txtmaTL.selectAll();
					txtmaTL.requestFocus();
					return false;
				} else if (TL.isMaTLTonTai(maTl)) {
			        JOptionPane.showMessageDialog(this, "Mã TL đã tồn tại. Vui lòng chọn mã khác.", "Thông báo lỗi", JOptionPane.ERROR_MESSAGE);
			        txtmaTL.selectAll();
			        txtmaTL.requestFocus();
			        return false;
				}
				else if (ten.trim().equals("")) {
					return showErrorTx(txtTenTL,"Tên TL không được để trống");
				} else if (TL.isTenTLTonTai(ten)) {
			        JOptionPane.showMessageDialog(this, "Tên TL đã tồn tại. Vui lòng chọn mã khác.", "Thông báo lỗi", JOptionPane.ERROR_MESSAGE);
			        txtTenTL.selectAll();
		...
		//Hàm tìm kiếm
			...
			private void timTheoTen() throws SQLException {
				...
				TheLoai_DAO daoTL=new TheLoai_DAO();
				List<TheLoai> dstl = TL.gettltheoTen(ten);
				if (dstl.size() == 0) {
					JOptionPane.showMessageDialog(null, "Không tìm thấy tên TL  theo ten! ");
